...
namespace SevenDigital.ApiInt.ServiceStack.Unit.Tests.Services
{
	...
	public class Given_voucher_has_not_been_applied
	{
		...
		public void then_if_basket_has_a_value_gt_zero_it_throws_expected_error()
		{
			...
			applyVoucher.Stub(x => x.Please()).Return(new ApplyVoucherToBasket() { AmountDue = new AmountDue { Amount = "0.99", FormattedAmount = "£0.99" } });
			...
			var voucherRequest = new VoucherPurchaseRequest { VoucherCode = expectedVoucherCode, Type = PurchaseType.release};
